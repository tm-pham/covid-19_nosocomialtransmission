...
# ============================#
# Health-care workers
N_hcw <- rep(1000, max_time)                                                     # Total number of HCWs at time t
...
discharged_dead_pat <- matrix(rep(1, max_time*max_time), ncol=max_time) 
...
for(t in 2:max_time){
  ...
  for(s in 2:t){
    ...
    new_symptomatic_hcw[s,t] <- min(hcw_symp, I_hcwU[s-1,t-1])
    # Remaining unknown infected HCWs at time t who got infected s-1 days ago
    I_hcwU[s,t] <- max(I_hcwU[s-1,t-1] - hcw_symp, 0)
    ...
    # Number of unisolated infected patients who were infected s days ago and are discharged or die
    discharged_dead_pat[s,t] <- outcomes[1,1]
  ...
  if("extraDistr"%in%installed.packages()){
    ...
    obs_hcw_infections[t] <- rbbinom(1, size=sum(new_symptomatic_hcw[1:S,t]), alpha=alpha_obs_hcw, beta=beta_obs_hcw)
  }else{
    print("No package for beta-binomial distribution installed. Aborting simulation...")
    break;
  }
  ...
  isolated_hcw[t] <- max(isolated_hcw[t-1] - hcw_recover + sum(new_symptomatic_hcw[1:(t-1),t]), 0)

  # Number of recovered HCWs
  R_hcw[t] = R_hcw[t-1] + hcw_recover
  # Number of susceptible HCWs
  S_hcw[t] = max(N_hcw[t] - R_hcw[t] - I_hcwU[1,t] - isolated_hcw[t], 0)
...
sim_data <- list(T=max_time, 
                 ...
                 i_pU0 = I_pU[1,1], 
                 new_symptomatic_pat0 = new_symptomatic_pat,
                 discharged_dead_pat0 = discharged_dead_pat,
                 N_hcw=N_hcw, 
                 hcw_isolation_period=hcw_isolation_period, 
                 i_hcwU0 = I_hcwU[1,1],
                 isolated_hcw0 = isolated_hcw[1],
                 new_symptomatic_hcw0 = new_symptomatic_hcw,
                 r_hcw0=R_hcw[1],
                 delta=delta, 
                 gen_shape=gen_shape,
                 gen_scale=gen_scale,
                 ...
                 p_hcw_obs=p_hcw_obs,
                 I_pU0_array = I_pU[1,], 
                 I_hcwU0_array = I_hcwU[1,])


saveRDS(sim_data, file="sim_data_v3.RDS")
