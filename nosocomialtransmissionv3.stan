...
data {
  ...
  real<lower=0> new_symptomatic_hcw0[T,T]; // Initial number of symptomatic HCWs at time 1
...
parameters {
  ...
  real <lower=0>hcw_symp[T,T];                 // Number of newly symptomatic HCWs
  real <lower=0>discharge_dead_pat[T,T];       // Number of patients that are discharged or die
  real <lower=0>symp_pat[T,T];                 // Number of patients that stay on the ward and develop symptoms
}
...
model{
  ...
  real new_symptomatic_pat[T,T];   // Number of unknown infected HCWs who got infected s-1 days ago and develop symptoms at time t
  real sum_symp_pat;
  ...
  new_symptomatic_pat[1,1] = new_symptomatic_pat0[1,1];
  // HCWs
  I_hcwU[1,1] = i_hcwU0;
  isolated_hcw[1] = isolated_hcw0;
  R_hcw[1] = r_hcw0;
  S_hcw[1] = fmax(N_hcw[1] - R_hcw[1] - I_hcwU[1,1] - isolated_hcw[1],eps);
  new_symptomatic_hcw[1,1] = new_symptomatic_hcw0[1,1];
  
  // Start loop
  for(t in 2:T){
    // Cumulative infectivity 
    inf_hcw[t-1] = dot_product(gen_time[1:(t-1)], I_hcwU[1:(t-1),t-1]);
    inf_p[t-1] = dot_product(gen_time[1:(t-1)], I_pU[1:(t-1),t-1]);
    ...
    S_p[t] = fmax(N_ncp[t] - I_pU[1,t], eps);
    ...
    for(s in 2:t){
      ...
      hcw_symp_gamma_shape = fmax(I_hcwU[s-1,t-1]*inc_distr[s]*hcw_symp_gamma_rate, eps);
      hcw_symp[s,t] ~ gamma(hcw_symp_gamma_shape,hcw_symp_gamma_rate);
      sum_symp_hcw = sum_symp_hcw + hcw_symp[s,t];
      // Remaining unknown infected HCWs at time t who got infected s-1 days ago
      new_symptomatic_hcw[s-1,t] = fmin(hcw_symp[s,t], I_hcwU[s-1,t-1]);
      I_hcwU[s,t] = fmax(I_hcwU[s-1,t-1] - hcw_symp[s,t], eps);
      ...
      p_d_gamma_shape = fmax(I_pU[s-1,t-1]*delta[s-1]*p_d_gamma_rate,eps);
      discharge_dead_pat[s,t] ~ gamma(p_d_gamma_shape,p_d_gamma_rate);
      // Unknown infected patients that develop symptoms
      temp_I_pU = fmax(I_pU[s-1,t-1] - discharge_dead_pat[s,t], eps);
      ...
      symp_pat[s,t] ~ gamma(p_symp_gamma_shape,p_symp_gamma_rate);
      sum_symp_pat = sum_symp_pat + symp_pat[s,t];
      // Remaining number of unknonwn unisolated patient at time t who got infected s-1 days ago
      new_symptomatic_pat[s-1,t] = fmin(symp_pat[s,t], temp_I_pU);
      I_pU[s,t] = fmax(temp_I_pU - symp_pat[s,t], eps);
    ...
    // ========================================================= //
    obs_nosocomial[t] ~ neg_binomial(p_p_obs*fmax(sum_symp_pat,eps), pDis_obs);
    obs_hcw_infections[t] ~ neg_binomial(p_hcw_obs*fmax(sum_symp_hcw,eps), pDis_obs);
    ...
    isolated_hcw[t] = fmax(isolated_hcw[t-1] - hcw_recover + sum(new_symptomatic_hcw[1:(t-1),t]), eps);

    // Number of recovered HCWs
    R_hcw[t] = R_hcw[t-1] + hcw_recover;
    // Number of remaining susceptible HCWs
    S_hcw[t] = fmax(N_hcw[t] - R_hcw[t] - I_hcwU[1,t] - isolated_hcw[t], eps);
  }
}
